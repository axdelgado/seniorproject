...
```{r}
# with entropy and gender
lm7 <- lm(log(SALARY) ~ GENDER + AGE + YRS_EXP + highest +
            RACE_SET + LOCATION_STATE + GENDER * d_hispanic, 
          data = alldata)
# summary(lm7)
```
...
```{r}
# with entropy and gender
lm9 <- lm(log(SALARY) ~ GENDER + AGE + YRS_EXP + highest +
            RACE_SET + LOCATION_STATE + GENDER * d_asian, 
          data = alldata)
# summary(lm9)
```
...
stargazer:: stargazer(
  ...
  covariate.labels = c("Female", "Age", "Years of Experience", 
                       "graduate degree", "Professional Degree", 
                       "Undergrad Degree", "Native American", "Asian", 
                       "Black", "Hispanic", "Entropy", "d_white", "d_black", 
