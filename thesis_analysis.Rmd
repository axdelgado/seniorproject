...
# loading data
load("~/statdiscrimination/alldata.RData")
...
## controls only
```{r}
lm1 <- lm(log(SALARY) ~ AGE + YRS_EXP + highest + GENDER + LOCATION_STATE, 
          data = alldata)
# summary(lm1)
```
...
## with race
```{r}
lm2 <- lm(log(SALARY) ~ AGE + YRS_EXP + highest + GENDER + RACE_SET +
            LOCATION_STATE, 
          data = alldata)
# summary(lm2)
```
...
```{r}
# with change in entropy
lm3 <- lm(log(SALARY) ~ d_entropy + AGE + YRS_EXP + highest + GENDER + 
...
lm4 <- lm(log(SALARY) ~ d_entropy * GENDER + AGE + YRS_EXP + highest +
            RACE_SET + LOCATION_STATE, 
          data = alldata)
# summary(lm4)
```
...
stargazer:: stargazer(
  ...
  notes = "Regression with a baseline of a white male with no more than a high 
  school education. Note: state effects are used but not shown.",
  notes.append = FALSE,
  notes.align = "l",
  single.row = FALSE,
  notes.label = ""
